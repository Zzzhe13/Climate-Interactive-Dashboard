// 初始化echart实例对象
var mapChart1 = echarts.init(document.getElementById('map1'), 'light', {
    width: 800,  // 设置图表宽度
    height: 600  // 设置图表高度
});
...
var option1 = {
    ...
    visualMap: { // 左侧小导航图标
        ...
        color: ["#355e3b","#008000", "#4cbb17","#90ee90","#ecffdc","#c9cc3f", ]
...
function updateCharts1(selectedYear) {
    ...
    // Update x-axis data
    // 获取中国各省市特区
    var provinces = data.areaTree[0].maphusummer
    ...
    for (var province of provinces) {
        ...
        option1.series[0].data.push({
            'name': province.name,
            'value': parseInt(province.avg + 10 * dp)
        })
    }
