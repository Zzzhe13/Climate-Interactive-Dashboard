// 初始化echart实例对象
var mapChart = echarts.init(document.getElementById('map1'), 'light', {
    width: 800,  // 设置图表宽度
    height: 500  // 设置图表高度
});
...
var option2 = {
    ...
    visualMap: { // 左侧小导航图标
        ...
        color: ["#33001a","#660033", "#cc0000", "#ff9900", "#ffcc80", "#cccc00", "#87cba8", "#006600"]
...
function updateCharts(selectedYear) {
    ...
    // Update x-axis data
    // 获取中国各省市特区
    var provinces = data.areaTree[0].mapaqi
    ...
    for (var province of provinces) {
        ...
        if (selectedYear != 2022){
            dp = (Math.random()*2-0.5)*30;
        }
        else dp = 0;
        option2.series[0].data.push({
            'name': province.name,
            'value': parseInt(province.avg + dp)
        })
    }
